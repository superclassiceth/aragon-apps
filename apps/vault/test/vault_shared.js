...
module.exports = (vaultName, { accounts, artifacts, web3 }) => {
  ...
  context(`> Shared tests for Vault-like apps`, () => {
    ...
    beforeEach(async () => {
      ...
      vault = await VaultLike.at(getNewProxyAddress(vaultReceipt))
    ...
    context('> Initialized', () => {
      ...
      context('> ETH', () => {
        ...
        it('fails if depositing a different amount of ETH than sent', async () => {
          ...
          await assertRevert(
            vault.deposit(ETH, value, { value: value * 2 }),
            errors.VAULT_VALUE_MISMATCH
          )
          assert.equal(
            (await getBalance(vault.address)).toString(),
            initialBalance,
            'vault should have initial balance'
          )
        })
        ...
        it('fails if not depositing any value', async () => {
          ...
          await assertRevert(
            vault.deposit(ETH, 0, { value: 0 }),
            errors.VAULT_DEPOSIT_VALUE_ZERO
          )
          assert.equal(
            (await getBalance(vault.address)).toString(),
            initialBalance,
        ...
        it('fails if depositing through non-registered function', async () => {
          ...
          await assertRevert(
            vault.sendTransaction({ data: '0xabcd1234', value: 1 }),
            errors.VAULT_DATA_NON_ZERO
          )
          assert.equal(
            (await getBalance(vault.address)).valueOf(),
            0,
        ...
        it('transfers ETH to EOA', async () => {
          ...
          const transferValue = web3.utils.toBN(10)

          await vault.sendTransaction({ value: depositValue })
          const testAccount = '0xbeef000000000000000000000000000000000000'
          const initialBalance = web3.utils.toBN(await getBalance(testAccount))

          // Transfer
          await vault.transfer(ETH, testAccount, transferValue)

          assert.equal(
            (await getBalance(testAccount)).valueOf(),
            initialBalance.add(transferValue),
            'should have sent eth'
          )
          assert.equal(
            (await getBalance(vault.address)).valueOf(),
        ...
        it('transfers ETH to contract', async () => {
          ...
          const initialBalance = web3.utils.toBN(
            await getBalance(destination.address)
          )
          ...
          assert.equal(
            (await getBalance(destination.address)).toString(),
            initialBalance.add(transferValue).toString(),
            'should have sent eth'
          )
          assert.equal(
            (await getBalance(vault.address)).toString(),
        ...
        it('fails if not transferring any vallue', async () => {
          ...
          await assertRevert(
            vault.transfer(ETH, ethReceiver, 0),
            errors.VAULT_TRANSFER_VALUE_ZERO
          )
          assert.equal(
            (await getBalance(vault.address)).toString(),
            initialBalance,
        ...
        it('fails transferring ETH if uses more than 2.3k gas', async () => {
          ...
          // Transfer
          await assertRevert(
            vault.transfer(ETH, destination.address, transferValue),
            errors.VAULT_SEND_REVERTED
          )
          assert.equal(
            (await getBalance(destination.address)).toString(),
            initialBalance,
            'destination should have initial balance'
          )
        })
      })
      ...
      for (const { title, tokenContract } of tokenTestGroups) {
        context(`> ERC20 (${title})`, () => {
          ...
          it('transfers tokens', async () => {
            ...
            assert.equal(
              await token.balanceOf(tokenReceiver),
              5,
          ...
          it('fails if not depositing any value', async () => {
            ...
            assert.equal(
              ...
              0,
              'vault should have initial token balance'
            )
          })
          ...
          it('fails if not transferring any vallue', async () => {
            ...
            await assertRevert(
              vault.transfer(token.address, tokenReceiver, 0),
              errors.VAULT_TRANSFER_VALUE_ZERO
            )
            assert.equal(
              await token.balanceOf(tokenReceiver),
              0,
              'receiver should have no token balance'
            )
          })
          ...
          it('fails if not sufficient token balance available', async () => {
            ...
            await assertRevert(
              ...
              errors.VAULT_TOKEN_TRANSFER_FROM_REVERT
            )
            assert.equal(
              await token.balanceOf(vault.address),
              0,
              'vault should have no token balance'
            )
          })
          ...
          it('fails deposits if token transfer not approved', async () => {
            ...
            // Attempt to deposit
            await assertRevert(
              vault.deposit(token.address, 10),
              errors.VAULT_TOKEN_TRANSFER_FROM_REVERT
            )
            assert.equal(
              await token.balanceOf(vault.address),
          ...
          it('fails transfers if token transfer fails', async () => {
            ...
            assert.equal(
              await token.balanceOf(tokenReceiver),
              0,
              'receiver should have initial token balance'
            )
            assert.equal(
              await token.balanceOf(vault.address),
              10,
      ...
      context('> Recovering assets', () => {
        ...
        beforeEach(async () => {
          ...
          const defaultVaultReceipt = await kernel.newAppInstance(
            vaultId,
            vaultBase.address,
            '0x',
            true
          )
          defaultVault = await VaultLike.at(
            getNewProxyAddress(defaultVaultReceipt)
          )
        ...
        xit('set up the default vault correctly to recover ETH from the kernel', async () => {
          ...
          assert.equal(
            (await getBalance(kernel.address)).valueOf(),
            0,
            'kernel should have 0 balance'
          )
          assert.equal(
            (await getBalance(defaultVault.address)).valueOf(),
            1,
            'default value should have 1 balance'
          )
        })
        ...
        it('set up the default vault correctly to recover tokens from the kernel', async () => {
          ...
          assert.equal(
            await token.balanceOf(kernel.address),
            10,
            'kernel should have 10 balance'
          )
          ...
          assert.equal(
            await token.balanceOf(defaultVault.address),
            10,
            'default value should have 10 balance'
          )
        })
    ...
    context('> Uninitialized', () => {
      ...
      context('> ETH', () => {
        it('fails to receive ETH deposits through fallback', async () => {
          await assertRevert(
            vault.sendTransaction({ value: 10 }),
            errors.INIT_NOT_INITIALIZED
          )
          assert.equal(
            (await getBalance(vault.address)).toString(),
            '0',
        ...
        it('fails to transfer ETH', async () => {
          ...
          await assertRevert(
            vault.transfer(ETH, ethReceiver, 10),
            errors.APP_AUTH_FAILED
          )
          assert.equal(
            (await getBalance(ethReceiver)).toString(),
            initialBalance,
            'receiver should have initial balance'
          )
        })
      })
      ...
      context('> ERC20', () => {
        ...
        it('fails to transfer tokens', async () => {
          ...
          await assertRevert(
            vault.transfer(token.address, tokenReceiver, 10),
            errors.APP_AUTH_FAILED
          )
          assert.equal(
            await token.balanceOf(tokenReceiver),
